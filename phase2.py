...
"""
============================================================================
SHIFAMIND PHASE 2: COMPLETE WORKING VERSION
============================================================================
ONE FILE - NO DEPENDENCIES - JUST RUN IT
============================================================================
"""

print("="*80)
print("🚀 PHASE 2: COMPLETE WORKING VERSION")
print("="*80)
...
PHASE2_CHECKPOINT_PATH = BASE_PATH / '03_Models/checkpoints/phase2_working'
PHASE2_RESULTS_PATH = BASE_PATH / '04_Results/experiments/phase2_working'
PHASE2_CHECKPOINT_PATH.mkdir(parents=True, exist_ok=True)
PHASE2_RESULTS_PATH.mkdir(parents=True, exist_ok=True)

PHASE1_CHECKPOINT = BASE_PATH / '03_Models/checkpoints/phase1_v3/phase1v3_stage3_joint.pt'

print(f"📁 Checkpoints: {PHASE2_CHECKPOINT_PATH}")
print(f"📁 Results: {PHASE2_RESULTS_PATH}")
...
df_train, df_temp = train_test_split(df_data, test_size=0.3, random_state=SEED, stratify=df_data['primary_dx'])
df_val, df_test = train_test_split(df_temp, test_size=0.5, random_state=SEED, stratify=df_temp['primary_dx'])
...
print("\n" + "="*80)
print("📚 BUILDING SIMPLE CORPUS")
print("="*80)

corpus_documents = []
corpus_metadata = []

# Simple clinical knowledge per diagnosis
clinical_knowledge = {
    'J189': [
...
# Add case prototypes from MIMIC
print("Adding case prototypes...")
for code in TARGET_CODES:
    cases = df_data[df_data['icd_codes'].apply(lambda x: code in x)]
    if len(cases) > 0:
...
print(f"\n✅ Built corpus: {len(corpus_documents)} documents")
...
# Save corpus
corpus_file = PHASE2_RESULTS_PATH / 'corpus.json'
...
# ============================================================================
# BUILD RETRIEVER & FAISS
# ============================================================================

print("\n" + "="*80)
print("🔍 BUILDING RETRIEVER & FAISS")
print("="*80)
...
print(f"✅ Checkpoint (F1: {checkpoint.get('macro_f1', 0):.4f})")
...
class ShifaMindPhase1(nn.Module):
    def __init__(self, base_model, num_concepts, num_classes, fusion_layers=[9, 11], use_pointer_network=True):
        super().__init__()
...
phase1_model = ShifaMindPhase1(
    ...
    num_classes=len(TARGET_CODES), fusion_layers=[9, 11], use_pointer_network=True
).to(device)
...
train_dataset = RAGDataset(df_train['text'].tolist(), df_train['labels'].tolist(),
                           tokenizer, rag_system, retrieve=True)
val_dataset = RAGDataset(df_val['text'].tolist(), df_val['labels'].tolist(),
                         tokenizer, rag_system, retrieve=True)
test_dataset = RAGDataset(df_test['text'].tolist(), df_test['labels'].tolist(),
                          tokenizer, rag_system, retrieve=True)
...
best_f1 = 0
checkpoint_file = PHASE2_CHECKPOINT_PATH / 'phase2_best.pt'
...
for epoch in range(num_epochs):
    ...
    print(f"  RAG Gate: {torch.sigmoid(phase2_model.rag_fusion.rag_gate).item():.4f}")
    ...
    if macro_f1 > 0.7509:
        print(f"  🔥 Beat Phase 1 by {macro_f1 - 0.7509:.4f}!")
...
improvement_pct = (improvement / phase1_f1) * 100
...
results_file = PHASE2_RESULTS_PATH / 'phase2_results.json'
